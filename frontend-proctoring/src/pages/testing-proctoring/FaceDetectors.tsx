...
const isLookingAway = (face: Face): boolean => {
  ...
  const rightEar = face.keypoints.find(
    (p: Keypoint) => p.name === "rightEarTragion"
  );
  const leftEar = face.keypoints.find(
    (p: Keypoint) => p.name === "leftEarTragion"
  );

  if (!rightEye || !leftEye || !noseTip || !rightEar || !leftEar || !face.box)
    return false;
  ...
  const earVisibilityRatio =
    Math.min(rightEarDist, leftEarDist) / Math.max(rightEarDist, leftEarDist); // ✅ (0-1)
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="flex flex-col items-center space-y-4">
          ...
          {modelReady ? (
            <>
              ...
              <p className="text-center">
                Number of faces detected: {faces.length}
              </p>
              <p></p>
              {/* ✅ Keypoints Table */}
              {faces.length > 0 && (
        ...
        <GestureDetector videoRef={videoRef} trigger={gestureTrigger} />
      </div>
    </div>
  );
};
